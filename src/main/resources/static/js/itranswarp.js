...
function deleteTopics(topicIds) {
  UIkit.modal.confirm(
    "Are you sure to delete " + topicIds.length + " topics?",
    function () {
      for (let i = 0; i < topicIds.length; i++) {
        let topicId = topicIds[i];
        setTimeout(function () {
          postJSON(
            "/api/topics/" + topicId + "/delete",
            {},
            function (err, resp) {
              if (err) {
                return showError(err);
...
function deleteTopicsAndLockUser(topicIds, userId) {
  UIkit.modal.confirm(
    "Are you sure to delete " + topicIds.length + " topics and lock user?",
    function () {
      _lockUser(userId);
      for (let i = 0; i < topicIds.length; i++) {
        let topicId = topicIds[i];
...
function deleteTopicAndLockUser(topicId, userId) {
  UIkit.modal.confirm(
    "Are you sure to delete this topic and lock user?",
    function () {
      _lockUser(userId);
      postJSON("/api/topics/" + topicId + "/delete", {}, function (err, resp) {
        if (err) {
...
function deleteReplyAndLockUser(replyId, userId) {
  UIkit.modal.confirm(
    "Are you sure to delete this reply and lock user?",
    function () {
      _lockUser(userId);
      postJSON("/api/replies/" + replyId + "/delete", {}, function (err, resp) {
        if (err) {
...
function _lockUser(userId) {
  postJSON(
    "/api/users/" + userId + "/lock/5000000000000",
    {},
    function (err, resp) {
      if (err) {
        return showError(err);
...
function ajaxLoadComments(insertIntoId, ref_id) {
  var errorHtml =
    'Error when loading. <a href="#0" onclick="ajaxLoadComments(\'' +
    insertIntoId +
    "', '" +
    ref_id +
    "')\">Retry</a>";
  ...
  $(function () {
    var isCommentsLoaded = false,
      ...
      checkOffset = function () {
        if (
          !isCommentsLoaded &&
          window.pageYOffset + window.innerHeight >= targetOffset
        ) {
...
function makeCollapsable(obj, max_height) {
  ...
  $o.parent().before(
    '<div class="x-anchor"><a name="' + aName + '"></a></div>'
  );
  var $p = $o.next();
...
function _adRandomMaterial(materials) {
  ...
  var weights = materials.map(function (m) {
      return m.weight;
    }),
    total_weights = weights.reduce(function (ax, w) {
      return ax + w;
    }, 0),
    rnd = Math.random(),
    ws = 0,
...
function initSponsors(ads) {
  ...
  var meta_tag = $('meta[property="og:tag"]').attr("content") || "",
  ...
  ads.slots.forEach(function (slot) {
    ...
    slot.periods.forEach(function (period) {
      var materials = _adFilterMaterials(meta_tags, period.materials),
    ...
    for (
      var i = 0;
      i < Math.min(slot.numAutoFill, slot.numSlots - slot.periods.length);
      i++
    ) {
